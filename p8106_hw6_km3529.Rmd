...
build 3 models. tells 2 models. come up with 1. 
compare 3 models using cross validation. 

interested in birthweight
...
2) One using head circumference, length, sex, and all interactions (including the three-way interaction) between these 
  - R assumes that if you put interaction between 2 var. it assumes you want main effect and adds that (birthweight against head cric times length times sex)
3) build your own! 


how to go about: 
1) if know have to do cross validation, start with given some model spec, it's possible to do cross validation (go to lecture. look at code. what do you needto change, output, dataset, model structure, model specifiction, output) try to make sure you cna get cross val to work ont his dataset 
2) just model building. take fulld ataset and thinka bout model you want to propose. start by picking var that might be interesting. look at results. are things significant. look at residuals. 
3) if can get cross valid ation to work for pre specfied models and then youc an (whatever using what) find collection of variables. then adding new model to cross validation results. 
There s achance you do all this and model doesn't work as well as 1st, yikes. should be similar or better than 2nd
